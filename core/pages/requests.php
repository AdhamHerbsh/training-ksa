        ...
        <div class="row justify-content-center">
            <div class="col-12">
                <div class="overlay-box p-3 p-sm-4 shadow rounded-4">
                    <div class="row">
                        <h2 class="h3 mb-0 text-white text-center mb-3" data-i18n="requests.title">Co-op Training
                            Requests</h2>
                    </div>
                        ...
                        <table class="table table-hover align-middle mb-0 text-white requests-table">
                            <thead class="sticky-top">
                                <tr class="align-middle">
                                    ...
                                    <th class="border-bottom border-secondary bg-dark bg-opacity-50"
                                        data-i18n="requests.id">National ID</th>
                                    <th class="border-bottom border-secondary bg-dark bg-opacity-50"
                                    ...
                                    <th class="border-bottom border-secondary bg-dark bg-opacity-50"
                                        data-i18n="requests.student-no">University ID</th>
                                    <th class="border-bottom border-secondary bg-dark bg-opacity-50"
                                    ...
                                    <?php foreach ($requests as $request): ?>
                                        <tr data-id="<?php echo htmlspecialchars($request['id']); ?>">
                                            ...
                                            <td class="border-bottom border-secondary">
                                                ...
                                                ?>
                                                <span
                                                    class="badge status-badge <?php echo $status_class; ?>"><?php echo htmlspecialchars($request['status']); ?></span>
                                            </td>
                                            <td class="border-bottom border-secondary">
                                                <div class="btn-group btn-group-sm">
                                                    ...
                                                    <button type="button" class="btn btn-outline-success accept-btn"
                                                        title="Accept" data-id="<?php echo htmlspecialchars($request['id']); ?>"
                                                        data-name="<?php echo htmlspecialchars($request['en_name']); ?>">
                                                        <i class="bi bi-check-lg"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-outline-danger reject-btn"
                                                        title="Reject" data-id="<?php echo htmlspecialchars($request['id']); ?>"
                                                        data-name="<?php echo htmlspecialchars($request['en_name']); ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Individual Accept/Reject button handlers
        studentsTableBody.addEventListener('click', function(event) {
            ...
            if (target.classList.contains('accept-btn')) {
                ...
                if (confirm(`Are you sure you want to accept the training request for "${name}"?`)) {
                    sendStatusUpdate([parseInt(id)], 'Accepted');
                }
            } else if (target.classList.contains('reject-btn')) {
                const id = target.dataset.id;
                const name = target.dataset.name;

                if (confirm(`Are you sure you want to reject the training request for "${name}"?`)) {
                    sendStatusUpdate([parseInt(id)], 'Rejected');
                }
            }
        });
        ...
        bulkAcceptBtn.addEventListener('click', function() {
            ...
            if (selectedIds.length > 0) {
                const message = selectedIds.length === 1 ?
                    'Are you sure you want to accept the selected request?' :
                    `Are you sure you want to accept ${selectedIds.length} selected requests?`;
        ...
        bulkRejectBtn.addEventListener('click', function() {
            ...
            if (selectedIds.length > 0) {
                const message = selectedIds.length === 1 ?
                    ...
                    `Are you sure you want to reject ${selectedIds.length} selected requests?`;
